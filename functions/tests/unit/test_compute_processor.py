...
class TestComputeInstanceProcessor:
    ...
    def test_can_process(self, pricing_service_mock, resource_spec_service_mock):
        processor = ComputeInstanceProcessor(
            pricing_service_mock, resource_spec_service_mock
        )
        assert processor.can_process("yandex_compute_instance") is True
        assert processor.can_process("other_resource") is False

    def test_process_standard_v3(
        self, usage_collector_mock, pricing_service_mock, resource_spec_service_mock
    ):
        processor = ComputeInstanceProcessor(
            pricing_service_mock, resource_spec_service_mock
        )
        ...
        resource = {
            ...
            "values": {
                ...
                "resources": [
                    {"cores": 2, "memory": 4, "core_fraction": 100, "gpus": 0}
                ],
                "scheduling_policy": [{"preemptible": False}],
                "network_interface": [{"nat": True}],
                "boot_disk": [
                    {"initialize_params": [{"size": 20, "type": "network-ssd"}]}
                ],
            },
        }
