...
class TestResourceSpecService:
    ...
    def sample_mdb_data(self):
        return {"mysql": {"s1.micro": {"cores": 1, "memory": 2, "core_fraction": 100}}}

    def test_get_mdb_preset(self, sample_mdb_data):
        service = ResourceSpecService(sample_mdb_data)
        cores, core_fraction, memory, platform_id = service.get_mdb_preset(
            "mysql", "s1.micro"
        )
        assert cores == 1
        assert memory == 2
    ...
    def test_platform_id_detection(self, sample_mdb_data):
        ...
        sample_mdb_data["mysql"]["s2.micro"] = {
            "cores": 1,
            "memory": 2,
            "core_fraction": 100,
        }
        ...
        sample_mdb_data["mysql"]["s3.micro"] = {
            "cores": 1,
            "memory": 2,
            "core_fraction": 100,
        }
