...
class TerraformCostEstimator:
    ...
    def __init__(self, pricing_service, resource_spec_service):
        ...
        self.processor_registry = ProcessorRegistry(
            self.pricing_service, self.resource_spec_service
        )
    ...
    def process_plan(self, tf_plan, param_full):
        ...
        if (
            "prior_state" in tf_plan
            and "values" in tf_plan["prior_state"]
            and "root_module" in tf_plan["prior_state"]["values"]
        ):
            prior_resources = tf_plan["prior_state"]["values"]["root_module"].get(
                "resources", []
            )
        ...
        if "planned_values" in tf_plan and "root_module" in tf_plan["planned_values"]:
            planned_resources = tf_plan["planned_values"]["root_module"].get(
                "resources", []
            )
