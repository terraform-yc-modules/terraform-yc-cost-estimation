...
def process_plan_command():
    ...
    parser.add_argument(
        "--full", action="store_true", help="Include detailed usage breakdown"
    )
    parser.add_argument(
    ...
    try:
        with open(args.json_file, "r") as f:
            ...
            if args.full and "current_usage" in result and "planned_usage" in result:
                ...
                common_keys = set(current_usage_dict.keys()) & set(
                    planned_usage_dict.keys()
                )
                ...
                if has_tabulate:
                    ...
                    for item in result["current_usage"]:
                        ...
                        if key in common_keys and not args.no_color:
                            if (
                                abs(
                                    current_usage_dict[key]["amount"]
                                    - planned_usage_dict[key]["amount"]
                                )
                                > 0.001
                            ):
                                # This resource has changed, highlight it
                                row = [
                                    f"{Colors.YELLOW}{cell}{Colors.RESET}"
                                    for cell in row
                                ]
                    ...
                    for item in result["planned_usage"]:
                        ...
                        if key in common_keys and not args.no_color:
                            if (
                                ...
                                # This resource has changed, highlight it
                                row = [
                                    f"{Colors.GREEN}{cell}{Colors.RESET}"
                                    for cell in row
                                ]
                ...
                if common_keys:
                    ...
                    if changes:
                        if has_tabulate:
                            ...
                            for current, planned in changes:
                                print(
                                    f"{current['resource_name']} ({current['resource_type']}): {current['amount']} → {planned['amount']} {current['unit']}, Cost: {current['cost']:.2f} → {planned['cost']:.2f} RUB/hour (Diff: {planned['cost'] - current['cost']:.2f})"
                                )
                    else:
